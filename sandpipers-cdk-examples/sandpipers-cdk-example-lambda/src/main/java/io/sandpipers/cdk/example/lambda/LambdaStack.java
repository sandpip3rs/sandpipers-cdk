...
import static io.sandpipers.cdk.example.lambda.TestLambdaUtils.getTestLambdaCodePath;
...
public class LambdaStack extends BaseStack {
  ...
  public LambdaStack(@NotNull final AbstractApp app, @NotNull final Environment environment) {
    super(app, environment);

    try {
      final String testLambdaCodePath = getTestLambdaCodePath(Path.of(System.getProperty("java.io.tmpdir")))
          .toFile().getPath();
      ...
      final CustomRuntime2023FunctionProps functionProps = CustomRuntime2023FunctionProps.builder()
          ...
          .environment(Map.of("ENV", "TEST", "SPRING_PROFILES_ACTIVE", "TEST"))
          .build();
      ...
      final LambdaRestApiProps restApiProps = LambdaRestApiProps.builder()
          .handler(function.getFunction())
          .deployOptions(StageOptions.builder().stageName("Test").build())
          .build();

      new io.sandpipers.cdk.core.construct.apigateway.LambdaRestApi(this, SafeString.of("RestApi"), restApiProps);
