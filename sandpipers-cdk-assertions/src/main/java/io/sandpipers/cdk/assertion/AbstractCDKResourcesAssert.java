...
public abstract class AbstractCDKResourcesAssert<SELF extends AbstractCDKResourcesAssert<SELF, ACTUAL>, ACTUAL extends Map<String, Object>> extends
  ...
  public SELF doCheckPolicyStatement(
    ...
    List<Map<String, Object>> statements = (List<Map<String, Object>>) policyDocument.get(
        "Statement");
        ...
        .anySatisfy(statement -> {
          Assertions.assertThat(statement)
              ...
              .containsEntry("Action",
                  isNotEmpty(actions) && actions.size() == 1 ? actions.get(0) : actions);
                ...
                .satisfies(res -> {
                  if (res instanceof String) {
                    Assertions.assertThat(res)
                        .isEqualTo(isNotEmpty(resource) && resource.size() == 1 ? resource.get(0)
                            : resource);
                  } else if (res instanceof Map) {
                    Assertions.assertThat((Map<String, String>) res)
                        .extracting("Ref")
                        .asString()
                        .matches(ref -> ref.matches(resource.get(0)));
                  }
                });
          }
          ...
          if (principal != null) {
            Assertions.assertThat(statement)
                ...
                .matches(e -> e.keySet().containsAll(principal.keySet()) && e.values()
                    .containsAll(principal.values()));
          }
        });
